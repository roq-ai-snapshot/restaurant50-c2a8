...
function TableAssignmentListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('table_assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/table-assignments/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('table_assignment', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Table Assignment
            </Text>
            <NextLink href={`/table-assignments/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('table_assignment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/table-assignments/edit/${record.id}`} passHref legacyBehavior>
                          <Button
