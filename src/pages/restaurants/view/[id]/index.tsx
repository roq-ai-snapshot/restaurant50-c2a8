...
import { deleteTableAssignmentById } from 'apiSdk/table-assignments';
...
function RestaurantViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RestaurantInterface>(
    ...
    () =>
      getRestaurantById(id, {
        relations: ['user', 'menu_item', 'table_assignment'],
      }),
  );
  ...
  const table_assignmentHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('table_assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Table Assignments
                    </Text>
                    <NextLink passHref href={`/table-assignments/create?restaurant_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.table_assignment?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/table-assignments/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.table_number}</Td>
                            <Td>
                              {hasAccess('table_assignment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/table-assignments/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('table_assignment', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    table_assignmentHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
